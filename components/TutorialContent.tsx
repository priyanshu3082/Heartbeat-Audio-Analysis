...
export default function TutorialContent() {
    const steps = [
        ...
        },
        {
            ...
            content:
                'If you used the built-in recorder on this site, the audio is already downloaded. If you recorded audio using other software, simply upload the file to the site. We support all audio formats.',
            image: Upload,
        },
    ];
    ...
    return (
        <div className="container mx-auto px-4 py-16">
            <div className="text-center mb-16 animate-fadeIn">
                <h1 className="text-5xl md:text-6xl font-extrabold mb-6 text-slate-900 dark:text-slate-50">Heartbeat Recording Protocol</h1>
                <div className="bg-white dark:bg-slate-900 rounded-xl p-6 max-w-3xl mx-auto border border-blue-100 dark:border-slate-700 shadow-sm">
                    <p className="text-lg text-slate-600 dark:text-slate-300">
                        If you already have an audio recording, then simply download it, but we recommend that you familiarize yourself
                        with it to know how to correctly record your heartbeat.
                    </p>
                </div>
            </div>
                ...
                {steps.map((stepItem, index) => (
                    ...
                    >
                        <div className="flex items-center space-x-4 mb-6">
                            <div
                                className={`w-20 h-20 rounded-2xl bg-gradient-to-br ${stepItem.color} flex items-center justify-center text-3xl font-bold text-white shadow-inner`}
                            >
                                {stepItem.step}
                            </div>
                            <div className="flex-1">
                                <h2 className="text-3xl font-bold text-slate-900 dark:text-slate-50">{stepItem.title}</h2>
                                <p className="text-slate-500 dark:text-slate-400">
                                    Step {stepItem.step} of {steps.length}
                                </p>
                            </div>
